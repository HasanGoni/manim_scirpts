...
class PaDiMSimple(Scene):
    """
    Simplified PaDiM Story without LaTeX dependencies
    ...
    def show_gaussian_modeling(self):
        ...
        # Statistical calculations explanation
        stats_explanation = VGroup(
            ...
            Text("   μ = (1/N) Σ x_i", font_size=12, color=SECONDARY_COLOR),
            Text("", font_size=6),  # Spacer
            Text("2. Covariance (Σ): Shape and orientation", font_size=14, color=SECONDARY_COLOR),
            Text("   Σ = (1/N-1) Σ (x_i - μ)(x_i - μ)ᵀ", font_size=12, color=SECONDARY_COLOR)
        ).arrange(DOWN, aligned_edge=LEFT, buff=0.1)
        stats_explanation.to_edge(RIGHT, buff=0.5)
        
        self.play(Write(stats_explanation, lag_ratio=0.2, run_time=3))
        self.wait(2)
        ...
        mean_label = Text("μ", font_size=16, color=ACCENT_COLOR, weight=BOLD)
    ...
    def show_anomaly_detection(self):
        ...
        formula = VGroup(
            ...
            Text("M(x) = √[(x - μ)ᵀ Σ⁻¹ (x - μ)]", font_size=24, color=SECONDARY_COLOR)
...
if __name__ == "__main__":
    ...
    # manim -pql padim_simple.py PaDiMSimple
    pass 